...
usage() {
    echo "[error] $1"
	echo "[info] usage: $0 -p <aws-profile> -pn <project-name> -e <environment> -c <component> -l <layer>"
	exit 1;
}
...
[[ -z "${VERSION}" ]] && { usage "No VERSION."; exit 1; }
...
# create bucket
#S3_BUCKET=hbs-cfn-${AWS_REGION}-${PROJECT_NAME}-${ENVIRONMENT}-${COMPONENT}-${LAYER}
S3_BUCKET=hbs-cfn-${PROJECT_NAME}-${ENVIRONMENT}-${COMPONENT}-${LAYER}
